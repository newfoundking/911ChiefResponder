...
function buildSegmentedImageData(labelMap, labels, sourceData, sourceWidth, sourceHeight, mapWidth, mapHeight, mode) {
  const out = new ImageData(sourceWidth, sourceHeight);
  let kept = 0;
...
async function segmentVehicle() {
  ...
  const rawSegmentationMap = loadedModel.predict(inputCanvas);
  const mapShape = rawSegmentationMap.shape || [];
  const mapHeight = mapShape[0] || 1;
  const mapWidth = mapShape[1] || 1;
  const labelMap = await rawSegmentationMap.data();
  rawSegmentationMap.dispose();

  const labels = deeplab.getLabels(modelQualitySelect.value);
  ...
  let { out, kept } = buildSegmentedImageData(
    labelMap,
    labels,
    sourceData,
    inputCanvas.width,
  ...
  if (needsFallback && mode === 'vehicle') {
    const fallback = buildSegmentedImageData(
      labelMap,
      labels,
      sourceData,
      inputCanvas.width,
